...
public class DatagramClient {
	public static void main(String[] args) {
		...
		DataInputStream dis = null;  // 보내는 것만...
		...
		try {
			...
			InetAddress iaddress = InetAddress.getByName(ipAddress);  // 문자열 ip주소를 실제 ip주소로 바꿔줌
			dp = new DatagramPacket(buffer, buffer.length, iaddress, port);  //(크기, 버퍼크기, 서버Ip, 포트번호)
			ds.send(dp);  // 송신
			...
			try { dis.close(); br.close(); } catch (IOException e) { e.printStackTrace(); }
		}
	}
}
